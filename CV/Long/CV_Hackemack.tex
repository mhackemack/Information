...
\begin{innerlist}
\item[] Worked on a team to develop a multiphysics analysis tool system for nuclear reactor core design. This work includes reactor physics and transport theory methods development, code implementation, reactor physics analysis, code verification and testing, user documentation creation, and user support. Furthermore, worked more integrally on a team subset to develop the deterministic radiation transport solver, Jaguar, which provides high-fidelity, 3D, $S_N$ solutions at scale on many parallel processes. Took over as the Jaguar team lead in 2020.
\end{innerlist}
...
\item \textbf{M.W. Hackemack}, ``Arbitrary-Order Extruded Bernstein-Bezier Functions for DGFEM Transport on 3D Prismatic Polygonal Grids,'' \textit{Journal of Nuclear Engineering} (Under Review).
