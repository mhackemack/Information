...
\vspace{-.325cm}
\begin{bibsection}
\item \textbf{M.W. Hackemack}, ``Quadratic Serendipity Functions for DGFEM Transport Solutions on Concave Polygonal Meshes,'' \textit{Transactions of the American Nuclear Society}, Orlando, Florida (Submitted).

\item \textbf{M.W. Hackemack} and J.C. Ragusa, ``Quadratic Serendipity Discontinuous Finite Element Discretization
for $S_N$ Transport on Arbitrary Polygonal Grids,'' \textit{Journal of Computational Physics}, (Accepted for Publication).
...
\vspace{-.325cm}
\begin{bibsection}
\item M.W. Hackemack, J.C. Ragusa, and J.E. Morel, \textit{Modifications to MIP DSA} (2018).

\item M.W. Hackemack, \textit{Scalable DSA Preconditioners} (2018).

\item M.W. Hackemack, \textit{Generalized Barycentric Coordinates for Diffusive Transport Problems} (2018).

\item M.W. Hackemack, \textit{Space-Energy Multigrid Acceleration} (2018).

\item M.W. Hackemack, \textit{Hierarchical Polygonal Functions} (2018).

\item M.W. Hackemack, \textit{Negative Flux Fixups for Nodal FEM} (2019).

\item M.W. Hackemack, \textit{Efficient Polygonal Quadrature} (2019).
